...
Texture::Texture(Texture&& _other) {
    mTexWidth = _other.mTexWidth;
    mTexHeight = _other.mTexHeight;
    this->mData = std::unique_ptr<uint32_t>(_other.mData.release());
}
...
Texture::Texture(const Texture&& _other) {
    ...
    mTexHeight = _other.mTexHeight;
    this->mData =
        std::unique_ptr<uint32_t>(const_cast<Texture&>(_other).mData.release());
}

Texture& Texture::addTexturePNG(const std::string& filePath,
    ...
    mData.release();
    mData = std::unique_ptr<uint32_t>(new uint32_t);

    pixels = loadPNG(filePath);
    for (int x = 0; x < mTexWidth; x++) {
        for (int y = 0; y < mTexHeight; y++) {
            RGBAPixel texel = pixels[mTexWidth * y + x];
            mData.get()[mTexWidth * y + x] =
                makeRGBA8888(texel.r, texel.g, texel.b, texel.a);
        }
    }

    this->mTexWidth = textWidth;
    this->mTexHeight = textHeight;
    return *this;
};

uint32_t Texture::operator[](size_t index) {
    _ASSERT(index < mTexHeight * mTexWidth);
    return mData.get()[index];
}

// idk
// void TextureList::addTexture(const Texture& tex_) {
//     mData.push_back(std::move(tex_));
// }

void TextureList::addTexture(const Texture&& tex_) {
    mData.emplace_back(std::move(tex_));
}
...
std::ostream& operator<<(std::ostream& stream, RGBAPixel pixel) {
    stream << " R: " << (int)pixel.r << " G: " << (int)pixel.g
        << " B: " << (int)pixel.b << " A: " << (int)pixel.a << std::endl;
    return stream;
}